...
Хотел на практике разобраться как несколько сервисов общаются через Kafka, работают с общей базой и запускаются вместе в Docker. По итогу пришлось разобраться с резервированием места, обработкой ошибок, доставкой файлов и развёртыванием на сервере.

Сначала я рассчитывал, что вся система поместится на сервере с 1 ГБ оперативной памяти. На практике четыре Spring приложения, Kafka и PostgreSQL оказались слишком тяжёлыми для такого VPS. Приложения долго запускались, а сервер переставал отвечать. Для стабильной работы пришлось перейти на 2 CPU и 4 ГБ RAM.

Что я понял: простое приложение не обязательно сразу делить на микросервисы. Для небольшого проекта модульный монолит предпочтительнее, а отдельные сервисы стоит добавлять, когда для этого появляется реальная причина.
